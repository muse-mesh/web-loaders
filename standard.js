...
// 1. Preconnect to CDN for faster font loading
const preconnectLink = document.createElement("link");
preconnectLink.rel = "preconnect";
preconnectLink.href = "https://cdn.jsdelivr.net";
document.head.appendChild(preconnectLink);

// 2. Inline critical loader styles for faster paint
const criticalLoaderCss = `
  ...
  .shadcn-loader-bar {
    height: 7px;
    background: repeating-linear-gradient(-45deg,#3c83f6 0 15px,#000 0 20px) left/200% 100%;
...
// 4. Defer font loading for loader (will be used by app anyway)
window.addEventListener("DOMContentLoaded", () => {
  shadcn_flutter.loadApp();
  shadcn_flutter_config.externalScripts.forEach((font) => {
  ...
  constructor({
    ...
    if (this.backgroundColor == null) {
      this.backgroundColor =
        localStorage.getItem("shadcn_flutter.background") || "#09090b";
    }
    if (this.foregroundColor == null) {
      this.foregroundColor =
        localStorage.getItem("shadcn_flutter.foreground") || "#ffffff";
    }
    if (this.loaderColor == null) {
      this.loaderColor =
        localStorage.getItem("shadcn_flutter.primary") || "#3c83f6";
    }
...
class ShadcnApp {
  ...
  loadApp() {
    ...
    window.addEventListener("shadcn_flutter_app_ready", () =>
      this.onAppReady()
    );
    window.addEventListener("shadcn_flutter_theme_changed", (event) =>
  ...
  onAppReady() {
    const loaderDiv = document.querySelector(".shadcn-loader-container");
    if (!loaderDiv) return;
    loaderDiv.style.opacity = 0;
    loaderDiv.style.pointerEvents = "none";
    window.requestAnimationFrame(() => {
      setTimeout(() => loaderDiv.remove(), this.config.transitionDuration + 50);
    });
  }
